 ...
 * @return void
 */
const https_server = function(logger, options) {
    ...
    this._server = https.createServer(options, this._app); // HTTP服务器
 ...
 * @return void
 */
https_server.prototype.bindHttpProtocols = function(path) {
    // 设置跨域
    let self = this;
 ...
 * @return void
 */
https_server.prototype.bindProtocol = function(proto) {
    let self = this;
    self._protoList['/' + proto.reqName] = proto;
    self._router.all('/' + proto.reqName, bodyParser.json(), function(request, response, next) {
        self._logger.info('/' + proto.reqName);
        self._logger.info(bodyParser.json());
        proto.handleProtocol(request, response);
    });
};
 ...
 * @return void
 */
https_server.prototype.start = function(port) {
    let self = this;
    ...
    self._logger.info("Https server start OK");
};
 ...
 * @return void
 */
https_server.prototype.update = function(time) {
 ...
 * @return void
 */
https_server.prototype.registerManager = function(manager) {
    manager.init(); // 初始化
    manager.load(); // 加载数据
 ...
 * @return void
 */
https_server.prototype.set = function() {
 ...
 * @return void
 */
https_server.prototype.use = function() {
